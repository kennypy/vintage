...
function ReviewsPage() {
  ...
  const { data, loading, error } = useApiQuery<Review[]>(
    userId ? `/reviews/${encodeURIComponent(userId)}` : null,
    { transform: unwrapList<Review> },
  );
  const reviews = data ?? [];
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      ) : error && reviews.length === 0 ? null : reviews.length === 0 ? (
