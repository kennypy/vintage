...
export default function ListingDetailClient({ id }: { id: string }) {
  ...
  useEffect(() => {
    apiGet<ListingData>(`/listings/${encodeURIComponent(id)}`)
      .then((data) => {
        setListing(data);
      })
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12">
        ...
        <div className="space-y-3">
          {/* Main image */}
          <div
            className={`relative aspect-[4/5] bg-gray-100 rounded-xl overflow-hidden ${hasImages ? 'cursor-zoom-in' : ''}`}
          ...
          {sellerName && (
            <div className="p-4 border border-gray-200 rounded-xl">
              <div className="flex items-center gap-3">
                <div className="relative w-10 h-10 bg-brand-100 rounded-full flex items-center justify-center text-brand-600 font-semibold text-sm overflow-hidden flex-shrink-0">
                  {listing.seller?.avatarUrl ? (
                    <Image src={listing.seller.avatarUrl} alt={sellerName} width={40} height={40} className="rounded-full object-cover" sizes="40px" />
                  ) : (
